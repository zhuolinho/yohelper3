...
// 负责人：李启波（marcyli）
//============================================================
- ( NSMutableDictionary *)sendPay_demo:(NSString *)order_name order_price:(NSString *)order_price notify_url:(NSString *)notify_url;
{
    ...
    if (notify_url.length + orderno.length > 32) {
        orderno = [orderno substringFromIndex:(notify_url.length + orderno.length - 32)];
    }
    orderno = [notify_url stringByAppendingString: orderno];
    NSLog(orderno);
    ...
    if ( prePayid != nil ) {
        ...
        [signParams setObject:orderno forKey:@"tradeNO"];
        //返回参数列表
        return signParams;
